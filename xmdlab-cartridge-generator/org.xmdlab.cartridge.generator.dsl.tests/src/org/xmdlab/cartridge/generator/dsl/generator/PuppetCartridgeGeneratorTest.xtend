...
class PuppetCartridgeGeneratorTest {
	...
	def void before() {
		val propertyFileContent = '''
			cartridge : {
				name : «modelName»
				basePackage : de.freund.xgen.cartridge
			}
		'''
		compilationTestHelper.copyToWorkspace("myProject/src/" + GeneratorConstants::CARTRIDGE_CONF_FILENAME,
			propertyFileContent)

		modelAsString = new Scanner(getClass.getResourceAsStream("/puppet/" + modelName + ".cartridge"), GeneratorConstants::ENCODING).
			useDelimiter("\\A").next();
	}
	...
	def void testGeneratedCode() {
		val String result = new Scanner(
			getClass.getResourceAsStream("/puppet/" + modelName + "-generator-test-result.txt"), GeneratorConstants::ENCODING).
			useDelimiter("\\A").next();
