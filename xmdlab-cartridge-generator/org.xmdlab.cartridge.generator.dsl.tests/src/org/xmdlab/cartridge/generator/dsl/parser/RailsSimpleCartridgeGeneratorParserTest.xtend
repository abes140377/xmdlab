...
class RailsSimpleCartridgeGeneratorParserTest {

	@Inject extension ParseHelper<DslModel>
	@Inject extension ValidationTestHelper

	val static cartridgeName = 'rails'

	var static String modelAsString = null

	@Before
	def void beforeClass() {
		modelAsString = new Scanner(getClass.getResourceAsStream("/rails/" + cartridgeName + ".cartridge"), GeneratorConstants::ENCODING).
			useDelimiter("\\A").next();
	}
