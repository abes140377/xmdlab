...
import org.xmdlab.dsl.application.applicationDsl.DslCommandEvent;
import org.xmdlab.dsl.application.applicationDsl.DslDataTransferObject;
import org.xmdlab.dsl.application.applicationDsl.DslDomainEvent;
...
public class DslHelper {
	...
	public static List<? extends DslSimpleDomainObject> getSubclasses(
	...
	public static List<DslDomainObject> getSubclasses(
			DslDomainObject domainObject) {
		List<DslDomainObject> subclasses = new ArrayList<DslDomainObject>();
		DslApplication application = (DslApplication) domainObject.eContainer()
				.eContainer();
		List<DslModule> modules = application.getModules();
		for (DslModule module : modules) {
			List<DslSimpleDomainObject> domainObjects = module
					.getDomainObjects();
			for (DslSimpleDomainObject simpleDomainObj : domainObjects) {
				if (simpleDomainObj instanceof DslDomainObject) {
					...
					if (domainObj.getExtendsName() != null
							&& domainObj.getExtendsName().equals(
									domainObject.getName())) {
						subclasses.add(domainObj);
					} else if (getExtends(domainObj) != null
							&& getExtends(domainObj).equals(domainObject)) {
						subclasses.add(domainObj);
					}
				}
			}
	...
	public static DslSimpleDomainObject getExtends(
			DslSimpleDomainObject domainObject) {
		...
		if (domainObject instanceof DslEntity) {
			...
			extendsName = ((DslDataTransferObject) domainObject)
					.getExtendsName();
		}

		if (result == null && extendsName != null) {
			DslApplication application = (DslApplication) domainObject
	...
	public static List<? extends DslSimpleDomainObject> getAllSubclasses(
			DslSimpleDomainObject domainObject) {
		if (domainObject instanceof DslDomainObject) {
	...
	public static List<DslDataTransferObject> getAllSubclasses(
			DslDataTransferObject domainObject) {
		List<DslDataTransferObject> subclasses = getSubclasses(domainObject);
		for (DslDataTransferObject subclass : new ArrayList<DslDataTransferObject>(
				subclasses)) {
	...
	public static List<DslDataTransferObject> getSubclasses(
			DslDataTransferObject domainObject) {
				...
				if (simpleDomainObj instanceof DslDataTransferObject) {
					...
					if (domainObj.getExtendsName() != null
							&& domainObj.getExtendsName().equals(
									domainObject.getName())) {
						subclasses.add(domainObj);
					} else if (domainObj.getExtends() != null
							&& domainObj.getExtends().equals(domainObject)) {
						subclasses.add(domainObj);
					}
				}
			}
	...
	public static List<DslDomainObject> getAllSubclasses(
			DslDomainObject domainObject) {
		List<DslDomainObject> subclasses = getSubclasses(domainObject);
		for (DslDomainObject subclass : new ArrayList<DslDomainObject>(
				subclasses)) {
			subclasses.addAll(getAllSubclasses(subclass));
		}
		return subclasses;
	...
	public static DslSimpleDomainObject findDomainObjectByName(String name,
			DslApplication application) {
		List<DslModule> modules = application.getModules();
		for (DslModule module : modules) {
