 ...
 * Xtend Template: org.xmdlab.cartridge.generator.dsl.templates.generator.CartridgeGeneratorWorkflowTpl
 */
...
class JeeCartridgeGeneratorWorkflow extends CartridgeGeneratorWorkflow {
	 ...
	 * 
	 */
	protected def MmApplication transformModel(DslApplication dslApplication) {
		LOGGER.info("Transforming dslApplication " + dslApplication.name)

		// run transformation
		val JeeCartridgeTransformation transformation = injector.getInstance(JeeCartridgeTransformation)
		var transformedModel = transformation.transform(dslApplication) as MmApplication

		if (transformedModel == null) {
			XmdlabGeneratorContext.addIssue(
				new XmdlabGeneratorIssueImpl(Severity.ERROR,
					"Transformation and modification of model '" + dslApplication.name + "' failed"))
		}
